...
import { ISerpentaTraining } from "./interfaces/ISerpentaTraining.sol";
    ...
    ISerpentaTraining
{
    ...
    address override public serpenta;
    ...
    /// @inheritdoc ISerpentaTraining
    function enableTraining(uint256[] calldata ids) external override nonReentrant whenNotPaused {
        unchecked {
    ...
    /// @inheritdoc ISerpentaTraining
    function disableTraining(uint256[] calldata ids) external override nonReentrant whenNotPaused {
        uint256 len = ids.length;
    ...
    /// @inheritdoc ISerpentaTraining
    function getUserTokens(address user) external override view returns (uint256[] memory) {
        return _getUserInfo[user].ids;
    }

    /// @inheritdoc ISerpentaTraining
    function inTraining(uint256[] calldata ids) external override view returns (bool[] memory b) {
        uint256 len = ids.length;
        if (len == 0) return b;
    ...
    /// @inheritdoc ISerpentaTraining
    function inTrainingFor(uint256[] calldata ids) external override view returns (uint256[] memory ts) {
        uint256 len = ids.length;
        if (len == 0) return ts;
